...
$(document).ready(function(){
    ...
    $("input[name$='payment']").click(function() {
		var test = $(this).val();
			...
			$("#paypal_add").show();
			$("#swift_info").hide();
			$('#payment_add input').addClass('pageRequired');
			$('#swift_info input').removeClass('pageRequired');
				//$('#badd3').removeClass('pageRequired');
			}
			else{
				$("#swift_info").show();
				$("#paypal_add").hide();
				$('#payment_add input').removeClass('pageRequired');
				$('#swift_info input').addClass('pageRequired');
	...
	<?php }  ?>
      ...
      <div style="display: block;">
        <div class="top_button_box">
          ...
          <div class="point">:</div>
          <div class="shipping_in_button select1"><a style="width:auto; padding-left:5px; padding-right:5px;background: #eee;"  href="<?php echo base_url();?>user/manage_design/paymentSetting">Payment settings</a></div>
        </div>
        <div class="tab_mid_box1">
          <div class="earning_box_left" id='sf1'>
           <h1>CHOOSE AND UPDATE A PAYMENT METHOD</h1>
	   <?php
	   $attr = array('id'=>'payment_form');
	   echo form_open('user/manage_design/savePaymentMethod',$attr); ?>
	   <input type="hidden" name='designer' value="<?php echo $designer->id; ?>" />
       
           	<p><input type="radio" value="paypal" name="payment" <?php if($designer->pmt_method=='paypal') { echo "checked='checked'"; } ?>/> PayPal</p>
	   
       		<div id="paypal_add" <?php if($designer->pmt_method=='paypal'){ ?> style="display:block" <?php }else{?>style="display:none;"<?php } ?>>
				<p>Enter your Paypal address</p>
				<p><input type="text" name="paddress" class='name_input pageRequired' size="25" value="<?php if(isset($designer->paypal_address)) echo $designer->paypal_address; ?>" /></p>
	   		</div>
        
        	<p><input type="radio" value="swift" name="payment" <?php if($designer->pmt_method=='swift') { echo "checked='checked'"; } ?>/> International Transfer(SWIFT)</p>
	 			<div id="swift_info"  <?php if($designer->pmt_method=='paypal'){ ?> style="display:none" <?php }else{?>style="display:block;"<?php } ?>>
					<p>Enter SWIFT code</p>
					<p><input type="text" name="swift_code" class='name_input' size="25" value="<?php if(isset($designer->swift_code)) echo $designer->swift_code; ?>" /></p>
					<p>Enter your IBAN or Account Number</p>
					<p><input type="text" name="account_no" class='name_input'  size="25" value="<?php if(isset($designer->bank_account_no)) echo $designer->bank_account_no; ?>" /></p>
	   		</div>
			<input type="submit" class="save_button pmtmethod" value="&nbsp;">
	<?php echo form_close(); ?>
        </div>
